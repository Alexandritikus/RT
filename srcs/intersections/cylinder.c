...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/04/20 14:34:10 by oshudria          #+#    #+#             */
/*   Updated: 2017/06/13 12:14:11 by oshudria         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			intersect_cylinder_param(t_ray ray, t_intersect *j)
{
	...
	else
	{
		...
		j->point = vector_add(ray.origin,
					vector_mult_d(ray.direction, j->distance));
		j->u = j->point.x / (2.0) + 0.5;
		j->v = j->point.y / (2.0) + 0.5;
	}
}
...
static void		cylinder_disc(t_ray ray, t_intersect *j)
{
	t_intersect	iends;
	double		t;

	if (intersect_cyl_disc(cyl_disc(vector(0.0, 0.0, -(HEIGHT / 2.0)), RADIUS),
																ray, &iends))
	{
